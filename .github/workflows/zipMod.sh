...
echo "Reading whitelist...\n$(cat $startingDir/whitelist.txt)"
git ls-files | grep -Ef $startingDir/whitelist.txt | while read file; do cp --parents "$file" "$outputFolderName"; done
