Scriptname Toaster:Loads_v2:QST_PatcherCore extends Quest
...
Function PatchInNewPrimaryMode(FormList akPatchList, String asCaller)
    ...
    Message mesgSelected = akPatchList.GetAt(4) as Message
    ...
    FormList flstWeaponSelectedMessage = flstWeapon.GetAt(3) as FormList
    ...
    Else
        ...
        If !flstWeaponSelectedObjectMod
            ...
            bError = True
        ElseIf !((akPatchList.GetAt(2) as FormList).GetAt(0) as ObjectMod)
        ...
        If !flstWeaponSelectedMessage
            Debug.Trace("Loads_v2:PatchInNewPrimaryMode: "+asCaller+" patch form list "+akPatchList+" index 0 is not a primary mode master form list. Its index 3 should be a FLST but isn't.")
            bError = True
        ElseIf !((akPatchList.GetAt(3) as FormList).GetAt(0) as Message)
            Debug.Trace("Loads_v2:PatchInNewPrimaryMode: "+asCaller+" patch form list "+akPatchList+" index 0 is not a primary mode master form list. Its index 3 "+flstWeaponSelectedMessage+" should contain MESG but doesn't.")
            bError = True
        EndIf
    EndIf
    If !kywdPossible
    ...
    EndIf
    If !mesgSelected
        Debug.Trace("Loads_v2:PatchInNewPrimaryMode: "+asCaller+" patch form list "+akPatchList+" index 4 is not an MESG")
        bError = True
    EndIf
